<?php
    ...
    if(isset($_SESSION['pqsuser']) && $_SESSION['designation'] == "System"){
        ...
        $table = "
            <table id='CONSOLIDATED_$date' class='dataTable'>
                <tr>
                    <th>$date</th>
                </tr>
                <tr>
        ...
        while($row = mysqli_fetch_assoc($result)){
            ...
            $result2 = mysqli_query($conn, $sql);
            $row2 = mysqli_fetch_assoc($result2);
            $customer = isset($row2['company_name']) ? $row2['company_name'] : 0;

            if($lab == "Bacteriology"){
                $sampleLab = "bacteriology_samples";
            }
            else if($lab == "Entomology"){
                $sampleLab = "entomology_samples";
            }
            else if($lab == "Mycology"){
                $sampleLab = "mycology_samples";
            }
            else if($lab == "Nematology"){
                $sampleLab = "nematology_samples";
            }
            else if($lab == "Molecular"){
                $sampleLab = "molecular_samples";
            }
            else if($lab == "Tissue Culture"){
                $sampleLab = "tissue_culture_samples";
            }
            else if($lab == "Virology"){
                $sampleLab = "virology_samples";
            }

            $sql = "SELECT * FROM " . $sampleLab . " WHERE sample_code = '$sample_code'";
            $result2 = mysqli_query($conn, $sql);
            $row2 = mysqli_fetch_assoc($result2);
